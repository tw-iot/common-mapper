...
type Item struct {
	...
	StartAddress     int    `json:"start_address"`      //起始地址
	Quantity         string `json:"quantity"`           //偏移量，即采集长度也可对应tcp采集上的字段类型
	DataFormatLength int    `json:"data_format_length"` //数据格式长度
	Cycle            int    `json:"cycle"`              //采集此指标频率，单位ms，默认1000
}
